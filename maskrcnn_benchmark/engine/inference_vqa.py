...
def create_prompt_from_path_json(json_dict, labels, label_list, tokenizer, paths, mode='hybrid'):
    captions = []
    #import pdb; pdb.set_trace()
    ...
    for i, path in enumerate(paths):
        real_path = path[2:]
        caption = json_dict[real_path]['caption']
        captions.append(caption)
        start_i = 0
        end_i = 0
        objects_query = ""
        ...
        tokenized = tokenizer(caption, return_tensors="pt")
            # 虽然我的tokenized每张图不一样，但多个类的positivie都在一句话上，
            # 所以一次性调用create_positive_dict函数可以， object_query = caption
        # import pdb;pdb.set_trace()
        assert objects_query == caption, print(f'{objects_query} and \n {caption}')
        positive_map_token_to_label, positive_map_label_to_token = create_positive_dict(tokenized, tokens_positive,
                                                                           labels=labels[0])
            #all_positive_maps.append(positive_map_label_to_token)
        
        positive_maps.append(positive_map_label_to_token)
    
    return captions, positive_maps
...
def inference(
    ...
    ##############VQA Model Building##########
    # ckpt_dir = 'OFA/OFA-base'
    # tokenizer = OFATokenizer.from_pretrained(ckpt_dir, use_cache=True)
    # model_vqa = OFAModel.from_pretrained(ckpt_dir, use_cache=False)
    tokeniszer_bert = AutoTokenizer.from_pretrained("bert-base-uncased")
    ###########################################
    ...
    categories = dataset.categories()
    if task == "detection":
        all_queries, all_positive_map_label_to_token, labels, label_list = create_queries_and_maps_from_dataset(dataset, cfg)
    elif task == "grounding":
        all_queries = [None]
            ...
            if cfg.TEST.USE_MULTISCALE:
                ...
                for query_i in range(query_time):
                    ...
                    if task == "detection":
                        ...
                        #import pdb; pdb.set_trace()
                        #captions = [all_queries[query_i] for ii in range(len(targets))]
                        #captions = create_prompt(categories, cfg, tokenizer, model_vqa, paths)
                        #captions, counter = create_prompt_from_json(i, counter, json_dict, len(batch[3]), batch[3])
                        captions, positive_map_label_to_token = create_prompt_from_path_json(json_dict, labels, label_list, tokeniszer_bert, paths)
                        #positive_map_label_to_token = positive_map_label_to_token[query_i]
                        #positive_map_label_to_token = all_positive_map_label_to_token[query_i]
                    elif task == "grounding":
