...
DATASETMAP = {
    ...
    'covid': {'data_path':'DATA/COVID-19_Radiography_Dataset/Normal/images', 'anno_path':'DATA/COVID-19_Radiography_Dataset/annotations/test.json'},
    'wbc': {'data_path':"DATA/wbc/Dataset1/s_img", 'anno_path': 'DATA/wbc/Dataset1/annotations/test.json'},
    'isic': {'data_path':'DATA/ISIC2018/ISIC2018_Task1-2_Test_Input', 'anno_path':'DATA/ISIC2018/annotations/test.json'},\
    'busi': {'data_path':'DATA/Dataset_BUSI_with_GT/malignant', 'anno_path':'DATA/Dataset_BUSI_with_GT/annotations/test_mal.json'},
    'busi_benign': {'data_path':'DATA/Dataset_BUSI_with_GT/benign', 'anno_path':'DATA/Dataset_BUSI_with_GT/annotations/test_benign.json'},
    'ct_liver': {'data_path':'DATA/CHAOS/CT/LiverImages', 'anno_path':'DATA/CHAOS/CT/annotations/test.json'},
    'mri_t1':{'data_path':'DATA/CHAOS/MRI/T1-DUAL/T1LiverImages', 'anno_path':'DATA/CHAOS/MRI/T1-DUAL/annotations/test_t1.json'},
    'mri_t2':{'data_path':'DATA/CHAOS/MRI/T2-SPIR/T2LiverImages', 'anno_path':'DATA/CHAOS/MRI/T2-SPIR/annotations/test_t2.json'},
}
...
def gpt4(concepts):
    ...
    context_q = "what does the liver look like in CT, describe it's shape and color"
    ...
    # lease all three most common values for each attribute'
        test_text = f'Based on the previous context infomation. Please provide a detailed description of the color in grayscale, location in the image, and shape attributes for the visual features of {concepts}.'+ \
    'For example, you should return an output following this format:{"color":["light gray", "dark", "white"], "shape":["round", "oval", "wedge shape"], loc:["upper right", "upper top", "center right"]}. Just use comma to seperate different attribute value, do not use "or" \
    ...
    # 'For example, you should return an output following this format:{"color":["pale white", "red", "pink"], "shape":["round", "oval"], loc:["colon"]}. Just use comma to seperate different attribute value, do not use "or" \
    # lease all three most common values for each attribute. Try to use common words rather than some advanced words for the attributes.'
            test_text = f'Please provide a detailed description of the color in grayscale, location, and shape attributes for the visual features of {concept}.'+ \
    'For example, you should return an output following this format:{"color":["light gray", "dark", "white"], "shape":["round", "oval", "wedge shape"], loc:["upper right", "upper right", "near center"]}. Just use comma to seperate different attribute value, do not use "or" \
    lease all three most common values for each attribute. Try to use common words rather than some advanced words for the attributes.'
        ...
        if len(concept) <= 1:
            ...
            try:
                ...
                # for k, (color, shape) in enumerate(zip(colors, shapes)):
                    caps[k] = f"large bright white color wedge shape area"#f'white color {shape} shape {real_names[0]} on {loc}' 
            ...
            for ipos, response in enumerate(responses):
                ...
                try:
                    ...
                    for k, (color, shape, loc) in enumerate(zip(colors, shapes, new_locs)):
                        ...
                        caps[ipos][k] = f'{color} {shape} {real_names[ipos]} on {loc}'#with {temp_b[ipos]} edge'# on {loc}'
                except:
